...
class QueueWorker:
    ...
    def run_once(self) -> bool:
        if self.queue.pending.empty():
            return False
        task_id = self.queue.pending.get()
        ...
        try:
            ...
            req = TextToVideoRequest(
                ...
                duration=float(metadata.get("duration", 4)),
                fps=int(metadata.get("fps", 24 if metadata.get("quality_preset") == "short_drama" else 12)),
                num_frames=metadata.get("num_frames"),
                seed=metadata.get("seed", 42),
                guidance_scale=float(metadata.get("guidance_scale", 7.5)),
                negative_prompt=str(metadata.get("negative_prompt", "")),
                aspect_ratio=metadata.get("aspect_ratio", "9:16" if metadata.get("quality_preset") == "short_drama" else "16:9"),
                resolution=metadata.get("resolution", "1080p" if metadata.get("quality_preset") == "short_drama" else "480p"),
                camera_motion=metadata.get("camera_motion", "slow_push_in"),
                quality_preset=metadata.get("quality_preset", "short_drama"),
            )
            result = generate_text_to_video(req)
            self.queue.store.update_task(
                task_id,
                status="completed",
                progress=1.0,
                output_video_path=result.output_path,
                metadata={**metadata, "result": result.model_dump()},
            )
            if task.project_id and task.shot_id:
                try:
                    from app.projects.project_manager import get_project_manager

                    get_project_manager().update_shot(
                        task.project_id,
                        task.shot_id,
                        {"status": "completed", "output_video_path": result.output_path},
                    )
                except Exception:
                    pass
